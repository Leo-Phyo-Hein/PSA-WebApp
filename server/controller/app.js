...
const pon = require("../model/pon.js")              // Load code from user.js
...
app.post("/users/", (req, res) => {
    ...
    user.addUser(username, password, role, driverNo, (err, result) => {
...
app.post("/request/", (req, res) => {
    ...
    pon.addPon(company_name, vehicle_number, driver_name, driverNo, item_details, image, userid, approval, (err, result) => {
        // Checking for errors. Output erorr code 500 if error detected
        if (!err) {
            // Responds by sending the result insertId (primary key) via a JSON string 
            console.log("Inserted ponid: " + result.insertId)
            res.status(201).send({'id': result.insertId})   // Return error code 201
        } else if (err.errno === 1062) {
            // Checking for duplicate error of input data
            console.log("[ERROR DETECTED] 422")
            console.log("[422] Duplicate Entry Detected.")
...
app.get("/ponperuser/:id", verifyToken, (req, res) => {
    // Retrieve the userid from the request parameters
    ...
    if (req.decodedToken.userid != userid) {
        ...
        pon.getponByID(userid, (err, result) => {
            // Check for errors
            if (!err) {
                res.status(200).send(result)
            } else {
...
app.put("/ponedit/:id", upload.single('image'), (req, res) => {
    ...
    var file = req.body.image
        ...
        if (req.file) {
            ...
            var file = 'http://localhost:8081/' + req.file.originalname.toLowerCase().split(' ').join('-')
        ...
        pon.updatepon(company_name, vehicle_number, driver_name, driverNo, item_details, file, id, (err, result) => {
            // Check for errors
            if (!err) {
...
app.put("/ponapprove/:id", (req, res) => {
    ...
    var approval = req.body.approval
        ...
        // Initiate function to update user by userid
        pon.approvepon(approval, id, (err, result) => {
            // Check for errors
            if (!err) {
                res.status(204).send(result)
            } else if (err.errno == 1062) {
...
app.get("/ponsign/", (req, res) => {
    // Perform function to retrieve all users from user database
    pon.getAllPons1((err, result) => {
        if (!err) {
            res.status(200).send(result)
        } else {
            console.log("[ERROR DETECTED] 500")
...
app.get("/pontransport/", (req, res) => {
    var driverNo = req.body.driverNo
    pon.getAllPons2(driverNo, (err, result) => {
        if (!err) {
            res.status(200).send(result)
        } else {
            console.log("[ERROR DETECTED] 500")
...
app.put("/ponremove/:id", (req, res) => {
    var id = req.params.id
    
        // Initiate function to update user by userid
        pon.removepon(id, (err, result) => {
            // Check for errors
            if (!err) {
                res.status(204).send(result)
            } else if (err.errno == 1062) {
...
app.get("/users/:id", verifyToken, (req, res) => {
    ...
    if (req.decodedToken.userid != userid) {
        ...
        user.getAllUsers(userid, (err, result) => {
            // Check for errors
            if (!err) {
                res.status(200).send(result)
            } else {
...
app.post("/login", (req, res) => {
    ...
    user.userLogin(username, password, (err, result) => {
        if (err) {
            res.status(500).send({ "Error Message": "[500] Unknown Error" })
            return
        } if (result === null) {
